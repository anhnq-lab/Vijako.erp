...
const initialApprovals: Approval[] = [
    { id: '1', type: 'Vật tư', code: '#MAT-203', title: 'Thép Hòa Phát D20', amount: '85tr', subtitle: 'Skylight City - Zone B', isUrgent: true },
    { id: '2', type: 'Thanh toán', code: '#PAY-882', title: 'Bê tông Việt Úc', amount: '120tr', subtitle: 'Đợt 2 - HĐ 22/HĐMB' },
    { id: '3', type: 'Nghỉ phép', code: '#HR-005', title: 'Trần Thị B', amount: '2 ngày', subtitle: 'Lý do: Việc gia đình' },
    { id: '4', type: 'Thiết bị', code: '#EQP-112', title: 'Thuê cẩu 50 tấn', amount: '15tr', subtitle: 'Phục vụ lắp dựng kèo thép', isUrgent: true },
];

const initialTasks: Task[] = [
    { id: 't1', title: 'Giao ban công trường', sub: 'Họp điều phối nhà thầu phụ tại Zone A', time: '08:00', status: 'done', type: 'meeting' },
    { id: 't2', title: 'Nghiệm thu thép sàn tầng 5', sub: 'Kiểm tra khoảng cách đai, móc neo theo TCVN', time: '09:30', status: 'active', type: 'site', file: 'Ban_ve_san_T5_Rev02.pdf', priority: 'high' },
    { id: 't3', title: 'Kiểm tra an toàn giàn giáo', sub: 'Khu vực mặt ngoài trục 1-4 (Giáo Ringlock)', time: '14:00', status: 'pending', type: 'site' },
    { id: 't4', title: 'Ký duyệt hồ sơ thanh toán', sub: 'Tổng hợp khối lượng tuần 3 tháng 10', time: '16:30', status: 'pending', type: 'doc' },
];
...
const TimelineTask: React.FC<TimelineTaskProps> = ({ task, onComplete }) => {
    ...
    return (
        <div className="relative pl-8 group mb-6 last:mb-0">
                    ...
                    <div className="w-full">
                        ...
                        {task.file && (
                            <div className="flex items-center gap-3 p-2.5 bg-slate-50 rounded-lg border border-slate-200 mt-3 hover:bg-white hover:border-primary/30 transition-colors cursor-pointer group/file">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-xs font-bold text-slate-900 truncate group-hover/file:text-primary transition-colors">{task.file}</p>
...
const NoteWidget = () => {
    const [note, setNote] = useState("- Gọi lại cho anh Bình bên Bê tông Việt Úc trước 3h chiều.\n- Kiểm tra lại bản vẽ Shop tầng 5.");
...
export default function Workspace() {
    ...
    const [selectedApprovals, setSelectedApprovals] = useState<string[]>(['1', '2']); // Select first two by default
    ...
    const handleBatchApprove = () => {
        ...
        // Simulate API Call
        const remaining = approvals.filter(a => !selectedApprovals.includes(a.id));
    ...
    const handleTaskComplete = (id: string) => {
        setTasks(prev => prev.map(t => {
            ...
            // Auto-activate next task logic could go here
            return t;
        }));
    };
    ...
    const filteredTasks = activeTab === 'all' ? tasks : tasks.filter(t =>
        activeTab === 'site' ? t.type === 'site' : t.type === t.type // simplified for demo
    );
        ...
        <div className="h-full flex flex-col bg-[#f7f7f8]">
            ...
            <header className="h-16 bg-white border-b border-slate-100 flex items-center justify-between px-6 shrink-0 z-10 sticky top-0">
                ...
                <div className="flex items-center gap-6">
                    ...
                    >
                        ...
                        <span className="text-xs font-bold">{isCheckedIn ? 'Đã Check-in (07:45)' : 'Chưa Check-in'}</span>
            ...
            <main className="flex-1 overflow-y-auto p-6 scroll-smooth">
                <div className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    {/* Left: Approvals (Batch Action) */}
                    <section className="lg:col-span-3 flex flex-col gap-4 h-fit sticky top-6">
                    ...
                    <section className="lg:col-span-6 flex flex-col gap-6">
                        ...
                        <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex-1 flex flex-col">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h3 className="text-xl font-bold text-slate-900">Việc hôm nay</h3>
                                    <p className="text-sm text-slate-500">Bạn đã hoàn thành <span className="font-bold text-primary">{taskDoneCount}/{taskTotalCount}</span> đầu việc quan trọng</p>
                                </div>
                                <div className="flex gap-2">
                                    <div className="flex bg-slate-100 rounded-lg p-1 gap-1">
                                        <button
                                            onClick={() => setActiveTab('all')}
                                            className={`px-3 py-1 text-xs font-bold rounded ${activeTab === 'all' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                                        >Tất cả</button>
                                        <button
                                            onClick={() => setActiveTab('site')}
                                            className={`px-3 py-1 text-xs font-bold rounded ${activeTab === 'site' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                                        >Hiện trường</button>
                                    </div>
                                    <button className="size-8 rounded hover:bg-slate-100 flex items-center justify-center text-slate-500"><span className="material-symbols-outlined">add</span></button>
                                </div>
                            </div>
